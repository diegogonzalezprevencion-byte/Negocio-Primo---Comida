...
const STORAGE_KEY = "chefOscar.menu.v3";
...
try {
  realtimeChannel = "BroadcastChannel" in window ? new BroadcastChannel(CHANNEL_NAME) : null;
} catch (error) {
...
let memoryMenuItems = null;
function safeStorageGet(storage, key) {
  try { return storage ? storage.getItem(key) : null; } catch (error) { return null; }
}
function safeStorageSet(storage, key, value) {
  try { if (storage) storage.setItem(key, value); return true; } catch (error) { return false; }
}
function safeStorageRemove(storage, key) {
  try { if (storage) storage.removeItem(key); return true; } catch (error) { return false; }
}
...
function getMenuItems() {
  if (Array.isArray(memoryMenuItems) && memoryMenuItems.length) return memoryMenuItems.map(normalizeItem);
  try {
    const raw = safeStorageGet(getLocalStorage(), STORAGE_KEY);
...
function saveMenuItems(items) {
  memoryMenuItems = items.map(normalizeItem);
  safeStorageSet(getLocalStorage(), STORAGE_KEY, JSON.stringify(memoryMenuItems));
  notifyMenuChanged();
}
...
function showAdminPanel(show) {
  ...
  document.body.classList.toggle("admin-unlocked", !!show);
  if (lock) {
    lock.hidden = !!show;
    lock.style.display = show ? "none" : "block";
  }
  if (panel) {
    panel.hidden = !show;
...
function initBackroom() {
  ...
  showAdminPanel(safeStorageGet(getSessionStorage(), SESSION_KEY) === "1");
  if (els.unlockBtn) els.unlockBtn.dataset.backroomBound = "true";

  els.unlockBtn?.addEventListener("click", () => {
    const typedPin = String(els.pinInput?.value || "").trim();
    if (typedPin === BACKROOM_PIN) {
      safeStorageSet(getSessionStorage(), SESSION_KEY, "1");
      showAdminPanel(true);
      setAdminMessage("Backroom habilitado.");
    } else {
      setAdminMessage("PIN incorrecto. Verifica la clave de administración.", "error");
    }
  });
  ...
  els.logoutBtn?.addEventListener("click", () => {
    safeStorageRemove(getSessionStorage(), SESSION_KEY);
  ...
  els.resetBtn?.addEventListener("click", () => {
    ...
    memoryMenuItems = null;
    safeStorageRemove(getLocalStorage(), STORAGE_KEY);
    notifyMenuChanged();
    renderAdmin();
...
window.CHEF_OSCAR_BACKROOM_READY = true;
window.CHEF_OSCAR_ADMIN_URL = "backroom.html";
